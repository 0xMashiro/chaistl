...
//     values.
//   - Iterators are constant iterators: mutating a key in place would silently
//     break the ordering invariant.
//   - The balancing scheme is a trailing policy parameter, defaulting to
//     rb_tree_policy.
//
// Standardization archaeology:
//   - set was standardized in C++98 as the canonical ordered unique-key
//     container, complementing map's key/value form.
//   - Its central rule is equivalence by comparator, not operator==: two keys
//     are equivalent when neither compares less than the other.
//   - C++17 node handles made it possible to extract and reinsert nodes while
//     preserving allocated storage and allowing controlled key mutation outside
//     the tree.
//   - C++23 added range insertion for the associative container family.
//
// Non-standard extensions:
//   - The Policy template parameter exposes rb/avl/treap experiments while
//     leaving set<Key> source-compatible with the standard shape.
//
// References:
...
/**
 * @brief Ordered unique-key associative container.
 *
...
class set {
 public:
  ...
  // iterators — keys are immutable once inserted, otherwise the ordering
  // invariant could be silently broken through `*it = x`. Making them the
  // same type (as libstdc++ does) keeps overload sets simple.
  ...
  // `set<K>::insert_return_type` (not a template).
  ...
  explicit constexpr set(const Compare& comp, const Allocator& alloc = Allocator{}) : tree_(comp, alloc) {}

  explicit constexpr set(const Allocator& alloc) : tree_(alloc) {}

  template <std::input_iterator InputIt>
  constexpr set(InputIt first, InputIt last, const Compare& comp = Compare{}, const Allocator& alloc = Allocator{})
      : tree_(first, last, comp, alloc) {}

  template <std::input_iterator InputIt>
  constexpr set(InputIt first, InputIt last, const Allocator& alloc) : tree_(first, last, Compare{}, alloc) {}

  template <concepts::container_compatible_range<value_type> R>
  constexpr set(std::from_range_t, R&& range, const Compare& comp = Compare{}, const Allocator& alloc = Allocator{})
      : tree_(comp, alloc) {
    insert_range(std::forward<R>(range));
  }

  template <concepts::container_compatible_range<value_type> R>
  constexpr set(std::from_range_t, R&& range, const Allocator& alloc)
      : set(std::from_range, std::forward<R>(range), Compare{}, alloc) {}

  constexpr set(std::initializer_list<value_type> init,
                const Compare& comp = Compare{},
                const Allocator& alloc = Allocator{})
      : tree_(init, comp, alloc) {}

  constexpr set(std::initializer_list<value_type> init, const Allocator& alloc) : tree_(init, Compare{}, alloc) {}

  constexpr set(const set& other) = default;
  constexpr set(const set& other, const std::type_identity_t<Allocator>& alloc) : tree_(other.tree_, alloc) {}

  constexpr set(set&& other) = default;
  constexpr set(set&& other, const std::type_identity_t<Allocator>& alloc) : tree_(std::move(other.tree_), alloc) {}
  ...
  constexpr set& operator=(set&& other) = default;
  constexpr set& operator=(std::initializer_list<value_type> init) {
    tree_ = init;
    return *this;
  }
  ...
  {
    CHAI_HARDENED(pos != end(), "chaistl::set::erase: invalid iterator");
  ...
  // [set.modifiers]: only the comparator may differ — the allocator type is
  // fixed, because spliced nodes keep their original allocation and must be
  // deallocatable by this container's allocator.
  template <class Compare2>
  constexpr void merge(set<Key, Compare2, Allocator, Policy>& source) {
  ...
  constexpr void swap(set& other) noexcept(noexcept(tree_.swap(other.tree_))) { tree_.swap(other.tree_); }
 ...
 private:
  ...
  // merge() splices nodes between sets that may differ in comparator type,
  ...
  template <class K2, class C2, class A2, class P2>
  friend constexpr bool operator==(const set<K2, C2, A2, P2>& lhs, const set<K2, C2, A2, P2>& rhs);

  template <class K2, class C2, class A2, class P2>
  friend constexpr auto operator<=>(const set<K2, C2, A2, P2>& lhs, const set<K2, C2, A2, P2>& rhs);
};
...
set(InputIt, InputIt, Allocator) -> set<std::iter_value_t<InputIt>, std::less<std::iter_value_t<InputIt>>, Allocator>;
...
[[nodiscard]] constexpr bool operator==(const set<Key, Compare, Allocator, Policy>& lhs,
                                        const set<Key, Compare, Allocator, Policy>& rhs) {
  return lhs.tree_ == rhs.tree_;
}

template <class Key, class Compare, class Allocator, class Policy>
[[nodiscard]] constexpr auto operator<=>(const set<Key, Compare, Allocator, Policy>& lhs,
                                         const set<Key, Compare, Allocator, Policy>& rhs) {
  return lhs.tree_ <=> rhs.tree_;
}
...
template <class Key, class Compare, class Allocator, class Policy>
constexpr void swap(set<Key, Compare, Allocator, Policy>& lhs,
                    set<Key, Compare, Allocator, Policy>& rhs) noexcept(noexcept(lhs.swap(rhs))) {
  lhs.swap(rhs);
}
...
// the corresponding opt-in header (e.g. <chaistl/containers/treap_set.hpp>).
// ============================================================================

template <class Key, class Compare = std::less<Key>, class Allocator = allocator<Key>>
using rb_set = set<Key, Compare, Allocator, detail::tree::rb_tree_policy>;

namespace pmr {

template <class Key, class Compare = std::less<Key>>
using set = chaistl::set<Key, Compare, chaistl::pmr::polymorphic_allocator<Key>>;
