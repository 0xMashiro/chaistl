...
// ============================================================================
// skip_list_set - Probabilistically balanced ordered unique-key set
// ============================================================================
//
// A skip list keeps several forward lists over the same sorted keys. Level 0
// stores every key; higher levels store a sampled subsequence, giving expected
// logarithmic search/update without rotations. This implementation uses a
// deterministic per-container xorshift generator so tests and examples are
// reproducible while the structure still illustrates probabilistic balancing.
//
// References:
// - William Pugh, Skip Lists: A Probabilistic Alternative to Balanced Trees.
// - dsacpp Skiplist/Quadlist implementation.
...
class skip_list_set {
 ...
 public:
  ...
  constexpr skip_list_set() : skip_list_set(Compare{}, Allocator{}) {}

  explicit constexpr skip_list_set(const Compare& comp, const Allocator& alloc = Allocator{})
      : compare_(comp), alloc_(alloc) {}

  explicit constexpr skip_list_set(const Allocator& alloc) : skip_list_set(Compare{}, alloc) {}

  template <std::input_iterator InputIt>
  constexpr skip_list_set(InputIt first,
                          InputIt last,
                          const Compare& comp = Compare{},
                          const Allocator& alloc = Allocator{})
      : skip_list_set(comp, alloc) {
    insert(first, last);
  }

  template <std::input_iterator InputIt>
  constexpr skip_list_set(InputIt first, InputIt last, const Allocator& alloc)
      : skip_list_set(first, last, Compare{}, alloc) {}

  template <std::ranges::input_range R>
    requires std::convertible_to<std::ranges::range_reference_t<R>, value_type>
  constexpr skip_list_set(std::from_range_t,
                          R&& range,
                          const Compare& comp = Compare{},
                          const Allocator& alloc = Allocator{})
      : skip_list_set(comp, alloc) {
  ...
  template <std::ranges::input_range R>
    requires std::convertible_to<std::ranges::range_reference_t<R>, value_type>
  constexpr skip_list_set(std::from_range_t, R&& range, const Allocator& alloc)
      : skip_list_set(std::from_range, std::forward<R>(range), Compare{}, alloc) {}

  constexpr skip_list_set(std::initializer_list<value_type> init,
                          const Compare& comp = Compare{},
                          const Allocator& alloc = Allocator{})
      : skip_list_set(init.begin(), init.end(), comp, alloc) {}

  constexpr skip_list_set(std::initializer_list<value_type> init, const Allocator& alloc)
      : skip_list_set(init.begin(), init.end(), Compare{}, alloc) {}

  constexpr skip_list_set(const skip_list_set& other) : skip_list_set(other.compare_, other.get_allocator()) {
    insert(other.begin(), other.end());
  }

  constexpr skip_list_set(const skip_list_set& other, const std::type_identity_t<Allocator>& alloc)
  ...
  constexpr skip_list_set& operator=(const skip_list_set& other) {
  ...
  constexpr skip_list_set& operator=(std::initializer_list<value_type> init) {
    clear();
  ...
  constexpr std::pair<iterator, bool> insert(const value_type& value) { return insert_impl(value); }
  constexpr std::pair<iterator, bool> insert(value_type&& value) { return insert_impl(std::move(value)); }

  constexpr iterator insert(const_iterator, const value_type& value) { return insert(value).first; }
  constexpr iterator insert(const_iterator, value_type&& value) { return insert(std::move(value)).first; }
  ...
  constexpr iterator erase(const_iterator position) {
    CHAI_HARDENED(position != end(), "chaistl::skip_list_set::erase: invalid iterator");
    const value_type& key = *position;
    auto next = position;
    ++next;
    erase(key);
    return next;
  ...
  constexpr size_type erase(const key_type& key) {
    auto update = search_predecessors(key);
    node* candidate = next_after(update[0], 0);
    if (candidate == nullptr || !equivalent(candidate->value, key)) {
      return 0;
    }

    for (std::size_t level = 0; level < level_count_; ++level) {
      if (next_after(update[level], level) != candidate) {
        break;
      }
      set_next_after(update[level], level, candidate->next[level]);
    }
  ...
  [[nodiscard]] constexpr iterator upper_bound(const key_type& key) noexcept {
    ...
    while (it != end() && equivalent(*it, key)) {
      ++it;
    }
    return it;
  }
  ...
  [[nodiscard]] constexpr std::pair<iterator, iterator> equal_range(const key_type& key) noexcept {
    ...
    auto last = first;
    if (last != end() && equivalent(*last, key)) {
      ++last;
    }
    return {first, last};
  }
 ...
 private:
  ...
  [[nodiscard]] constexpr node* predecessor_at_level(node* start, std::size_t level, const key_type& key) const {
    node* current = start;
  ...
  [[nodiscard]] constexpr std::array<node*, max_level> search_predecessors(const key_type& key) const {
    std::array<node*, max_level> update{};
    node* current = nullptr;
    for (std::size_t level = level_count_; level-- > 0;) {
      current = predecessor_at_level(current, level, key);
  ...
  [[nodiscard]] constexpr unsigned next_random_bit() noexcept {
    // Fixed seed keeps tests reproducible; this is not intended to resist
    // adversarial insertion orders that can exploit deterministic structure.
    rng_state_ ^= rng_state_ << 13;
    rng_state_ ^= rng_state_ >> 7;
    rng_state_ ^= rng_state_ << 17;
    return static_cast<unsigned>((rng_state_ >> 63) & 1u);
  }
  ...
  constexpr std::pair<iterator, bool> insert_impl(Value&& value) {
    auto update = search_predecessors(value);
    node* candidate = next_after(update[0], 0);
    if (candidate != nullptr && equivalent(candidate->value, value)) {
      return {iterator(candidate), false};
    }

    const auto height = random_height();
    const auto old_level_count = level_count_;
    ensure_levels(height);
    for (std::size_t level = old_level_count; level < height; ++level) {
...
constexpr void swap(skip_list_set<Key, Compare, Allocator>& lhs,
                    skip_list_set<Key, Compare, Allocator>& rhs) noexcept(noexcept(lhs.swap(rhs))) {
  lhs.swap(rhs);
}
